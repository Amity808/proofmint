...
export function useENSProfile({ address, name, type = 'general' }: UseENSProfileOptions) {
    ...
    const isVerified = textRecords[ENS_TEXT_RECORDS.MERCHANT_VERIFIED] === 'true' ||
    ...
    const isLoading = isNameLoading || isAddressLoading || isAvatarLoading ||
        textRecordQueries.some(query => query.isLoading);
