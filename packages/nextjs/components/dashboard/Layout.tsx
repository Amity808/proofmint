...
const Layout: React.FC<LayoutProps> = ({ account }) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100">
      ...
      {/* Sidebar */}
      <div
        className={`fixed left-0 top-0 h-full w-72 bg-white/80 backdrop-blur-xl border-r border-slate-200/60 z-50 transform transition-transform duration-300 ${sidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"
          }`}
      >
          ...
          <nav className="flex-1 p-4 space-y-1">
            {menuItems.map((item, index) => (
              <NavLink
                ...
                to={item.path}
                className={({ isActive }) =>
                  `w-full flex items-center justify-between px-4 py-3 rounded-xl font-medium transition-all duration-200 ${isActive
