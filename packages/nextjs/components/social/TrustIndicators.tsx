...
import { AlertCircle, Award, CheckCircle, Clock, Heart, Shield, Star, TrendingUp, Users, Zap } from "lucide-react";
  ...
  if (variant === "minimal") {
    return (
      <div className="flex items-center gap-2">
        ...
        <div
          className={`px-2 py-1 rounded-full text-xs font-medium ${getScoreBgColor(trustScore.overall)} ${getScoreColor(trustScore.overall)}`}
        >
    ...
    return (
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <div
            className={`px-3 py-1 rounded-full text-sm font-medium ${getScoreBgColor(trustScore.overall)} ${getScoreColor(trustScore.overall)}`}
          >
            {trustScore.overall}% Trust
          </div>
        </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div
                    className={`h-2 rounded-full ${getScoreBgColor(trustScore.verification)}`}
              ...
              <div>
                ...
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div
                    className={`h-2 rounded-full ${getScoreBgColor(trustScore.reputation)}`}
              ...
              <div>
                ...
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div
                    className={`h-2 rounded-full ${getScoreBgColor(trustScore.activity)}`}
                    style={{ width: `${trustScore.activity}%` }}
                  />
                </div>
              </div>
              <div>
                ...
                <div className="w-full bg-gray-200 rounded-full h-2">
                  <div
                    className={`h-2 rounded-full ${getScoreBgColor(trustScore.community)}`}
                    style={{ width: `${trustScore.community}%` }}
                  />
                </div>
              </div>
            </div>
          ...
          <div className="text-center">
            ...
            <div className="flex items-center justify-center gap-1">
              {[1, 2, 3, 4, 5].map(star => (
                <Star
                  key={star}
                  className={`w-5 h-5 ${star <= Math.floor(rating) ? "text-yellow-500 fill-current" : "text-gray-300"}`}
                />
              ))}
            </div>
          ...
          {badges.map((badge, index) => (
            ...
            >
              <div className="flex items-center gap-3">
                <div className="p-2 bg-white/50 rounded-lg">{getBadgeIcon(badge.type)}</div>
                <div>
