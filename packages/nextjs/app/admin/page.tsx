...
const AdminDashboard: React.FC = () => {
    ...
    const [activeTab, setActiveTab] = useState<"overview" | "users" | "merchants" | "recyclers" | "receipts" | "subscriptions" | "funds">("overview");
    ...
    const handlePauseMerchantSubscription = async (merchantAddress: string, pause: boolean) => {
        ...
        try {
            ...
            toast.success(`Merchant subscription ${pause ? 'paused' : 'unpaused'} successfully! Transaction hash: ${result}`, { id: "pause-subscription" });
        } catch (error) {
                        ...
                        <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-6">
                            ...
                            <div className="space-y-4">
                                {allReceipts.length === 0 ? (
                                    ...
                                    allReceipts.slice(-3).reverse().map((receipt: any, index: number) => (
                                            ...
                                            <div className="flex items-center space-x-3">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-sm text-gray-600">
                                                        {receipt.gadgetStatus === 0 ? 'Active' :
                                                            receipt.gadgetStatus === 1 ? 'Stolen' :
                                                                receipt.gadgetStatus === 2 ? 'Lost' :
                                                                    receipt.gadgetStatus === 3 ? 'Recycled' : 'Unknown'} gadget
                                                    </p>
                                                </div>
                                            </div>
                                            ...
                                            {allReceipts.map((receipt, index) => (
                                                <tr key={index} className="hover:bg-gray-50">
                                                    ...
                                                    <td className="px-6 py-4 whitespace-nowrap">
                                                        <span className={`px-2 py-1 text-xs font-medium rounded-full ${receipt.gadgetStatus === 0 ? 'bg-green-100 text-green-800' :
                                                            receipt.gadgetStatus === 1 ? 'bg-red-100 text-red-800' :
                                                                receipt.gadgetStatus === 2 ? 'bg-yellow-100 text-yellow-800' :
                                                                    receipt.gadgetStatus === 3 ? 'bg-purple-100 text-purple-800' :
                                                                        'bg-gray-100 text-gray-600'
                                                            }`}>
                                                            {receipt.gadgetStatus === 0 ? 'Active' :
                                                                receipt.gadgetStatus === 1 ? 'Stolen' :
                                                                    receipt.gadgetStatus === 2 ? 'Lost' :
                                                                        receipt.gadgetStatus === 3 ? 'Recycled' : 'Unknown'}
                                                        </span>
                                                    </td>
                                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                                        {new Date(Number(receipt.timestamp) * 1000).toLocaleDateString()}
