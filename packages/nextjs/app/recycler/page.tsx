...
const RecyclerDashboard: React.FC = () => {
  ...
  const availableForRecycling = dummyReceipts.filter(receipt => receipt.status !== GadgetStatus.Recycled);

  // Filter receipts that have been recycled
  const recycledItems = dummyReceipts.filter(receipt => receipt.status === GadgetStatus.Recycled);
        ...
        <div className="mb-8">
          <div className="relative">
            <h1 className="text-4xl font-bold text-gray-900 mb-2">
              <span className="brand-gradient-multi bg-clip-text text-transparent">Recycler Dashboard</span>
            </h1>
        ...
        {activeTab === "available" && (
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              ...
              <div className="text-sm text-gray-600">{availableForRecycling.length} items ready for processing</div>
            </div>
            ...
            {availableForRecycling.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {availableForRecycling.map(receipt => (
                  <div
                    key={receipt.id}
                    className="bg-white/80 backdrop-blur-sm rounded-xl shadow-brand-primary border border-brand-primary/20 overflow-hidden"
                  >
                    ...
                    <div className="p-6">
                      ...
                      <div className="space-y-2 text-sm text-gray-600 mb-4">
                        <p>
                          <span className="font-medium">Product ID:</span> {receipt.productId}
                        </p>
                        <p>
                          <span className="font-medium">Merchant:</span> {receipt.merchant}
                        </p>
                        <p>
